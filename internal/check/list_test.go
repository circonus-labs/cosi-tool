...
func TestGetDetail(t *testing.T) {
	...
	if _, err := getDetail(nil, "testdata/", "", uiURL, false); err == nil {
		t.Fatal("expected error")
	} else if err.Error() != "invalid check registration file (empty)" {
		t.Fatalf("unexpected error (%v)", err)
	}
	...
	if _, err := getDetail(client, "testdata/", "bad.json", uiURL, false); err == nil {
		t.Fatal("expected error")
	} else if err.Error() != "parsing check registration json: unexpected end of JSON input" {
		t.Fatalf("unexpected error (%v)", err)
	}

	t.Log("\tvalid regfile (no verify)")
	if _, err := getDetail(client, "testdata/", "registration-check-system.json", uiURL, false); err != nil {
		t.Fatalf("unexpected error (%v)", err)
	}

	t.Log("\tvalid regfile (verify)")
	if _, err := getDetail(client, "testdata/", "registration-check-system.json", uiURL, true); err != nil {
		t.Fatalf("unexpected error (%v)", err)
	}
}
